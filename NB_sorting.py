...
## load extra broad bands
#CFHT_Mr = responses[75277:75486]
#file_filter = open('/cosma/home/dp004/dc-armi2/PAU_test/filters/CFHT_MegaCam-iband_response_na.dat','a')
#for i in range(len(CFHT_Mr)):
#    file_filter.write(CFHT_Mr[i])
#file_filter.close()
...
for i in range(len(NB_names)):
    nb = np.loadtxt(NB_names[i])
    PAUS_NBs.append(nb)
    
c_nm = np.zeros(40)
for i in range(40):
    c_nm[i] = np.trunc(np.round(np.average(PAUS_NBs[i][:,0],weights=PAUS_NBs[i][:,1])/10.)/10.)
#### sort and save bands by central wavelength 
PAUS_NBs_sorted = np.array(PAUS_NBs)[np.argsort(c_nm)]
#
lini=4550
for i in range(40):
    nb = PAUS_NBs_sorted[i]
    new_nb = nb[np.where(nb[:,1]>1e-6)]
...
#removing 0s in tails
lini=4550
for i in range(40):
    nb = PAUS_NBs_sorted[i]
    new_nb = nb[np.where(nb[:,1]>1e-6)]
    np.savetxt('/cosma/home/dp004/dc-armi2/PAU_test/filters/PAUS_'+str(lini)+'_band_nt.dat',new_nb,fmt='%.7lf',header='wavelength response')
    lini+=100
