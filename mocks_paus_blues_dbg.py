...
#    F_in /= (1+table[i,2])
    for c in range(len(Ix)):
        if c == 0:
            if (0<=(Ix[c]- xi)<dx2):
                F_in[c] *= ((dx2 - (xi - Ix[c]))/dx)
            elif (0<=(xi - Ix[c])<dx2):
                F_in[c] *= ((dx2 - (xi - Ix[c]))/dx)
        if c == len(Ix) - 1:
            if (0<=(xf - Ix[c])<dx2):
                F_in[c] *= ((dx2 - (Ix[c]-xf))/dx)
            elif (0<=(Ix[c] - xf) < dx2):
                F_in[c] *= ((dx2 - (Ix[c]-xf))/dx)
...
t.write('/cosma/home/dp004/dc-armi2/PAU_test/catalogs/mocks_radecz_MIMB_SFRHaplha_'+icut+'cut_fix_nfrf.fits',format='fits')
